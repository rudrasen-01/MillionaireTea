...
function AppContent() {
  const { currentPage, setCurrentPage, isLoading, user, notifications, removeNotification, markNotificationRead, markAllNotificationsRead, darkMode, toggleDarkMode, setUser, fetchNotifications } = useApp();
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [showLanding, setShowLanding] = useState(false);
  const [isNotifOpen, setIsNotifOpen] = useState(false);
  const [showAllNotifDropdown, setShowAllNotifDropdown] = useState(false);
  ...
  const handleBellClick = async () => {
    ...
    // Reset expanded dropdown when opening
    if (willOpen) setShowAllNotifDropdown(false);
    if (willOpen && fetchNotifications) {
      await fetchNotifications();
    }
  };

  // Close notifications when clicking outside the dropdown or bell button
  useEffect(() => {
          ...
          >
            ...
            <div
              className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative"
                style={{
                  // Add extra right padding on small screens and when viewing the user panel
                  paddingRight: isSmallScreen ? (currentPage === 'user' ? '1.5rem' : '1rem') : undefined
                }}
              >
              <div className="flex items-center justify-between h-16">
                  ...
                  <motion.div
                    ...
                    className="p-2 rounded-lg"
                        style={{
                          ...
                          // ensure some right spacing so bell doesn't touch or overflow the viewport
                          marginRight: isSmallScreen ? '0.5rem' : (currentPage === 'user' ? '0.75rem' : undefined)
                        }}>
                ...
                <div className="flex items-center gap-3">
                        ...
                        {isNotifOpen && (
                          <>
                            ...
                            <motion.div
                              ...
                              className="absolute mt-2 rounded-2xl overflow-hidden z-40"
                              ref={notifRef}
                              style={{
                                background: darkMode ? '#1F2937' : 'white',
                                ...
                                maxHeight: '32rem',
                                // On small screens keep dropdown within viewport with side margins
                                position: isSmallScreen ? 'fixed' : 'absolute',
                                top: isSmallScreen ? '3.25rem' : undefined,
                                left: isSmallScreen ? '0.5rem' : undefined,
                                right: isSmallScreen ? '0.5rem' : '1rem',
                                width: isSmallScreen ? 'auto' : '24rem',
                                maxWidth: isSmallScreen ? 'calc(100% - 1rem)' : '24rem',
                                zIndex: 9999
                              }}
                            >
                              {/* Header */}
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                  ...
                                  {notifications && notifications.length > 0 && (
                                    <button
                                      ...
                                      style={{ color: darkMode ? '#9CA3AF' : '#B87333', background: 'transparent', border: 'none', cursor: 'pointer' }}
                                    >
                                      Mark all read
                                    </button>
                                  )}
                              ...
                              <div className="overflow-y-auto" style={{ maxHeight: '28rem' }}>
                                {(notifications || []).length === 0 ? (
                                  ...
                                  (() => {
                                    ...
                                    const display = showAllNotifDropdown ? sorted : sorted.slice(0, 3);
                                    return (
                                      <>
                                        {display.map((n) => {
                                      const id = n.id || n._id;
                                      ...
                                      if (fullText.toLowerCase().includes(teaUpdatePhrase.toLowerCase())) {
                                        ...
                                        preview = fullText.length > 80 ? fullText.slice(0, 80) + '...' : fullText;
                                      }
                                      ...
                                      return (
                                        ...
                                        {sorted.length > display.length && (
                                          <div className="px-4 py-2 text-right">
                                            <button
                                              onClick={() => setShowAllNotifDropdown(v => !v)}
                                              className="text-sm font-semibold text-caramel hover:underline"
                                            >
                                              {showAllNotifDropdown ? 'Show less' : `See more (${sorted.length - display.length})`}
                                            </button>
                                          </div>
                                        )}
                                      </>
                  ...
                  {/* Mobile Menu Button (grouped with bell on right side) */}
                  <button
