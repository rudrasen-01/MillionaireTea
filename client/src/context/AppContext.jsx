...
export function AppProvider({ children }) {
  ...
  // Only fetch dashboard/admin endpoints when an authenticated admin is present
  const fetchAppData = async () => {
    try {
      // Always fetch public rankings
      const ranksRes = await fetch('/api/rewards/rankings');
      if (ranksRes && ranksRes.ok) {
        const r = await ranksRes.json();
        dispatch({ type: 'SET_ALL_USERS', payload: r.rankings || [] });
      }

      // Dashboard is admin-only and requires authentication; avoid fetching it
      // when there is no authenticated admin user to prevent 401s.
      const isAdmin = state.user && (state.user.role === 'admin' || state.user.role === 'superadmin');
      if (isAdmin) {
        try {
          const dashRes = await fetch('/api/rewards/dashboard', { credentials: 'include' });
          ...
          // log but do not treat missing dashboard as fatal for non-admins
          console.error('Failed to fetch dashboard (admin-only)', e);
        }
      }
    } catch (e) {
  ...
  // Ensure admin dashboard data is fetched as soon as an admin signs in
  useEffect(() => {
    if (state.user && (state.user.role === 'admin' || state.user.role === 'superadmin')) {
      // fetch immediately to populate admin KPIs when user becomes admin
      fetchAppData();
    }
  }, [state.user]);
        ...
        if (state.user && String(notif.userId) === String(state.user._id || state.user.id)) {
          ...
          const payload = {
            ...
            read: false
                ...
                dispatch({ type: 'ADD_NOTIFICATION', payload });
                // If current user is an admin, optimistically update admin KPIs
                // so changes appear instantly, then refresh to reconcile.
                if (state.user && (state.user.role === 'admin' || state.user.role === 'superadmin')) {
                  try {
                    // Make a shallow copy and increment known KPI fields if present in the notification
                    const currentKPIs = state.adminKPIs || {};
                    const qty = Number(notif.quantity) || 0;
                    const amt = Number(notif.amount || notif.totalAmount || 0) || 0;
                    const updatedKPIs = { ...currentKPIs };
                    if (qty) {
                      const prev = Number(currentKPIs.totalTeasSold) || 0;
                      updatedKPIs.totalTeasSold = prev + qty;
                    }
                    if (amt) {
                      const prevR = Number(currentKPIs.totalRevenue) || 0;
                      updatedKPIs.totalRevenue = parseFloat((prevR + amt).toFixed(2));
                    }
                    // Optionally increment totalUsers or awards if payload indicates (safe-guarded)
                    if (notif.newUser && !isNaN(Number(notif.newUser))) {
                      const prevU = Number(currentKPIs.totalUsers) || 0;
                      updatedKPIs.totalUsers = prevU + Number(notif.newUser);
                    }

                    // Apply optimistic update
                    dispatch({ type: 'SET_ADMIN_KPIS', payload: updatedKPIs });
  ...
  const value = {
    ...
    markAllNotificationsRead: async () => {
      try {
        const ids = (state.notifications || []).map(n => n.id || n._id).filter(Boolean);
        // Optimistically mark all as read in state
        ids.forEach(id => dispatch({ type: 'MARK_NOTIFICATION_READ', payload: id }));

        // Send API requests in parallel
        await Promise.all(ids.map(async (id) => {
          const n = state.notifications.find(x => String(x.id || x._id) === String(id));
          try {
            if (n && n.adminNotification) {
              await fetch(`/api/rewards/admin/notifications/${id}/read`, { method: 'POST', credentials: 'include' });
            } else {
              await fetch(`/api/rewards/notifications/${id}/read`, { method: 'POST', credentials: 'include' });
            }
          } catch (e) {
            // ignore individual errors, we'll log
            console.error('markAllNotificationsRead individual error', e);
          }
        }));
      } catch (e) {
