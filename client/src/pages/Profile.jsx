...
export function Profile() {
  const { user, setUser, addNotification, notifications, fetchNotifications, setCurrentPage } = useApp();
  const timeAgo = (d) => {
    if (!d) return '';
  ...
  const [fetchedReviewCount, setFetchedReviewCount] = React.useState(null);
  React.useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        ...
        if (!mounted) return;
        setFetchedReviewCount((b.reviews || []).length);
      } catch (e) {
        // ignore
      }
    })();
  ...
  const totalReviews = (user && user.reviewCount != null)
    ? user.reviewCount
      ...
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="relative p-8">
            ...
            {/* Level Progress removed for all users */}
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Statistics Card - hidden for admin users */}
            {user?.role !== 'admin' && (
              <motion.div
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
          ...
          {/* Right Column - Additional Info (hidden for admins) */}
          {user?.role !== 'admin' && (
