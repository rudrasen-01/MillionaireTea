...
// ================= CURRENT USER =================
// Public-friendly current user endpoint:
// Returns the authenticated user when a valid token cookie is present,
// otherwise returns { user: null } with 200 to avoid noisy 401 responses
// on unauthenticated page loads (useful for SPA initial checks).
router.get('/me', async (req, res) => {
  try {
    ...
    // On unexpected errors, return null user but don't propagate 401 to client.
    console.error('GET /api/auth/me error:', err && err.stack ? err.stack : err);
    return res.json({ user: null });
  }
});
